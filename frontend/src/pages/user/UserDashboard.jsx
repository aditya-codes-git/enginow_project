    ...
    <main className="min-h-screen bg-theme-bg/50 p-6 md:p-8 text-theme-text font-sans">
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-6 justify-between">
            <div className="flex items-center gap-6">
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-theme-text-muted text-sm">
        ...
        <section className="grid gap-6 sm:grid-cols-2">
          <div className="bg-theme-surface rounded-2xl border border-theme-divider border-l-4 border-l-blue-600 p-6 shadow-sm flex flex-col justify-between min-h-[130px]">
            <div>
              <p className="text-theme-text-muted text-xs font-semibold uppercase tracking-wider">Registered Programs</p>
              {loading ? (
                <div className="h-9 w-16 bg-slate-200 animate-pulse rounded-lg mt-2" />
              ) : (
                <p className="text-4xl font-extrabold text-theme-text mt-2 font-outfit">{(events || []).length}</p>
              )}
            </div>
          ...
          <div className="bg-theme-surface rounded-2xl border border-theme-divider border-l-4 border-l-emerald-500 p-6 shadow-sm flex flex-col justify-between min-h-[130px]">
            <div>
              <p className="text-theme-text-muted text-xs font-semibold uppercase tracking-wider">Project Submissions</p>
              {loading ? (
                <div className="h-9 w-16 bg-slate-200 animate-pulse rounded-lg mt-2" />
              ) : (
                <p className="text-4xl font-extrabold text-theme-text mt-2 font-outfit">{(submissions || []).length}</p>
              )}
            </div>
            <p className="text-theme-text-secondary text-xs font-medium border-t border-slate-50 pt-3 mt-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <section className="space-y-4">
            <h2 className="text-2xl font-bold font-outfit text-theme-text">Your Registered Events</h2>
            <div className="space-y-4">
              {loading ? (
                <div className="space-y-4">
                  {[1, 2].map((i) => (
                    <div key={i} className="bg-theme-surface rounded-2xl border border-theme-border/80 p-5 shadow-sm space-y-3">
                ...
                (events || []).map((evt) => (
                  <div key={evt.id} className="bg-theme-surface rounded-2xl border border-theme-border/80 p-5 shadow-sm hover:shadow-md transition duration-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-blue-55 text-blue-700">{evt.type}</span>
                        <span className="text-xs font-semibold text-theme-text-secondary flex items-center gap-1">
                          <CalendarDays className="w-3.5 h-3.5" /> {formatDate(evt.startDate)}
                        </span>
                      </div>
                      <h3 className="font-bold text-lg text-theme-text">{evt.title}</h3>
                      <p className="text-theme-text-secondary text-sm max-w-md line-clamp-1">{evt.tagline || evt.description}</p>
                    </div>
                    <Link
                      to={evt.type === 'Hackathon' ? `/hackathons/${evt.slug || evt.id}` : `/events/${evt.slug || evt.id}`}
                      className="bg-theme-bg-secondary hover:bg-slate-200 text-theme-text-secondary font-semibold px-4 py-2 rounded-xl text-sm transition shrink-0"
                    >
                      View Details
                    </Link>
                  </div>
          ...
          {/* Submissions */}
          <section className="space-y-4">
            <h2 className="text-2xl font-bold font-outfit text-theme-text">Your Project Submissions</h2>
            <div className="space-y-4">
              {loading ? (
                <div className="space-y-4">
                  {[1, 2].map((i) => (
                ...
                (submissions || []).map((sub) => (
                  <div key={sub._id || sub.id} className="bg-theme-surface rounded-2xl border border-theme-border/80 p-5 shadow-sm space-y-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-xs font-bold text-theme-primary bg-blue-50 px-2.5 py-0.5 rounded-full">Project</span>
                        <h3 className="font-bold text-lg text-theme-text mt-1">{sub.title}</h3>
                      </div>
                      <span className="text-xs font-semibold text-theme-text-muted">
                        {formatDate(sub.createdAt)}
                      </span>
                    </div>
                    <p className="text-theme-text-secondary text-sm line-clamp-2">{sub.description}</p>
                    <div className="flex flex-wrap gap-3 pt-2">
                      {sub.githubUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1 text-theme-text-secondary hover:text-theme-primary font-semibold text-xs border border-theme-border rounded-lg px-2.5 py-1.5 transition"
                        >
