...
function CreateEventPage() {
  ...
  // Prefill organizer contact info from logged-in user
  useEffect(() => {
    if (user) {
      setFormData((current) => ({
        ...current,
  ...
  const missingBasics = useMemo(
    () => ['title', 'tagline', 'description', 'location', 'startDate', 'endDate', 'registrationDeadline'].filter((field) => !String(formData[field] || '').trim()),
    [formData],
  )
  ...
  const handleSubmit = async () => {
    if (missingBasics.length) {
      setError('Please add title, tagline, description, location, start/end dates, and registration deadline before saving.')
      setCurrentStep(0)
      return
    }
    ...
    try {
      ...
      const eventId = createdEvent?.id || createdEvent?._id || createdEvent?.data?.id || createdEvent?.data?._id;
  ...
  return (
    <main className="min-h-screen bg-neutral-50">
      <StepWrapper
        ...
        description="Create a complete event profile with full operational structure: public details, registrations, submissions, judging, and final review."
        onBack={handleBack}
        onNext={handleNext}
