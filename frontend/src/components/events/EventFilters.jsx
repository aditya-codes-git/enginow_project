    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-2 font-outfit text-2xl font-bold text-slate-950">{total} events found</h2>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3 lg:min-w-[680px]">
          ...
          <label className="grid gap-2 text-sm font-semibold text-slate-700">
            Status
            <select value={filters.status || 'All'} onChange={(event) => updateFilter('status', event.target.value)} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-slate-900 outline-none transition focus:border-blue-300 focus:bg-white focus:ring-4 focus:ring-blue-100">
              {statuses.map((status) => (
